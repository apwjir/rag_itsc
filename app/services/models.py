...
def find_model_by_display_name(display_name: str) -> LLMModel | None:
    """Find a model by its display name across all available models."""
    all_models = AVAILABLE_MODELS
    return next((model for model in all_models if model.display_name == display_name), None)
...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | None:
